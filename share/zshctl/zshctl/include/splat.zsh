# Currently creates two sub-processes to first parse the parameters and turn
# them into a token stream that is easily processed by a compiler and then
# evaluating the source generated by the compiler. It will not be assigning to
# any local variables, only expanding variables.
#
# The invocation, however, is going to suffer from shadowing.
...
function splat {
    ...
    set -- 0 "$@"
    while true; do
        if [[ ! -v "__splat_$1__" ]]; then
            break
        fi
        set -- $(( $1 + 1 )) "$@[2,-1]"
    done
    # Set to actual variable name.
    set -- "__splat_${1}__" "$@[2,-1]"
    function {
        typeset params=${1:-}
        shift
        typeset arguments=() stack=( "${(@Oa)@}" )
